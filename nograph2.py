...
class PollingThread(QThread):
    ...
    def __init__(self, client):
        super().__init__()
        self.client = client  # ✅ Store the passed-in LibreLinkUpClient instance
        ...
        try:
            ...
            for entry in new_data:
                ...
                insert_query = """
                    ...
                    VALUES (%s, %s)
                """
                cursor.execute(insert_query, (entry["value"], entry["timestamp"]))
                inserted_count += 1

            connection.commit()
            print(f"✅ Inserted {inserted_count} new glucose readings.")
...
class MainWidget(QWidget):
    ...
    def init_login(self):
        self.client = LibreLinkUpClient()  # Direct instantiation, no import needed

        while not self.client.login():
            pass
